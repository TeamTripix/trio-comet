...
import { dailyDealsProductSchema } from "../../../../models/dailyDealProducts";
    ...
    if (auth && auth.startsWith("Bearer")) {
      ...
      try {
        ...
        if (
          ...
          if (id !== "") {
            const updateProduct =
              await dailyDealsProductSchema.findOneAndUpdate(
                { _id: id },
                {
                  name,
                  description,
          ...
          const data = new dailyDealsProductSchema({
            name,
            description,
            price,
            discountPrice,
...
export async function GET(
  ...
  try {
    ...
    try {
      if (pid) {
        const response = await dailyDealsProductSchema.find({ _id: pid });
        return NextResponse.json(
          {
        ...
        const response = await dailyDealsProductSchema.find();
        return NextResponse.json(
          {
            message: "product found successfully",
            data: response,
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    try {
      ...
      try {
        const response = await dailyDealsProductSchema.deleteOne({ _id: id });
        return NextResponse.json(
